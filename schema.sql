...
CREATE TABLE IF NOT EXISTS users (
    ...
    vip_started_at INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS chat_activity (
    user_id INTEGER NOT NULL, chat_id INTEGER NOT NULL, day TEXT NOT NULL, messages INTEGER NOT NULL DEFAULT 0,
    PRIMARY KEY(user_id, chat_id, day), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS vip_task_claims (
    user_id INTEGER NOT NULL, task_key TEXT NOT NULL, claimed_at INTEGER NOT NULL,
    PRIMARY KEY(user_id, task_key), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS events (
    id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT NOT NULL, end_at INTEGER NOT NULL,
    reward_money INTEGER NOT NULL DEFAULT 0, reward_respect INTEGER NOT NULL DEFAULT 0, winners_count INTEGER NOT NULL DEFAULT 1,
    status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','finished','cancelled')), winner_ids TEXT NOT NULL DEFAULT '[]',
    created_by INTEGER NOT NULL, created_at INTEGER NOT NULL, FOREIGN KEY(created_by) REFERENCES users(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS event_participants (
    event_id INTEGER NOT NULL, user_id INTEGER NOT NULL, joined_at INTEGER NOT NULL,
    PRIMARY KEY(event_id,user_id), FOREIGN KEY(event_id) REFERENCES events(id) ON DELETE CASCADE, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
);
