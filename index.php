...
function vipText(int $id): string { global $db; if(!isVip($id)){ return "💎 <b>VIP</b>\n\nVIP-задания доступны только VIP-игрокам.\n\n🎯 5 работ → <b>4 500 бабок</b>\n🔪 3 ограбления → <b>10 000 бабок</b>\n💬 30 сообщений → <b>15 000 бабок + 10 Респектов</b>"; }$p=vipProgress($id);$q=$db->prepare('SELECT task_key FROM vip_task_claims WHERE user_id=?');$q->execute([$id]);$c=array_flip($q->fetchAll(PDO::FETCH_COLUMN));return "💎 <b>VIP-ЗАДАНИЯ</b>\n\n🎯 5 работ: <b>{$p['jobs']}/5</b> ".(isset($c['jobs'])?'✅':'')."\n🔪 3 ограбления: <b>{$p['robberies']}/3</b> ".(isset($c['robberies'])?'✅':'')."\n💬 30 сообщений: <b>{$p['messages']}/30</b> ".(isset($c['messages'])?'✅':''); }
...
function adminChatsText(): string {
    global $db;
    $rows=$db->query("SELECT chat_id,chat_type,title,username,active,last_seen_at FROM chat_registry WHERE chat_type IN ('group','supergroup') ORDER BY active DESC,last_seen_at DESC")->fetchAll();
    if(!$rows) return "💬 <b>ЧАТЫ БОТА</b>\n\nПока нет зарегистрированных групп.\nДобавь бота в группу и отправь там сообщение.";
    $s="💬 <b>ЧАТЫ БОТА</b>\n\n";
    foreach($rows as $r){
        $title=$r['title']?:($r['username']?'@'.$r['username']:'Без названия');
        $title=htmlspecialchars((string)$title,ENT_QUOTES|ENT_SUBSTITUTE,'UTF-8');
        $status=(int)$r['active']===1?'🟢':'🔴';
        $type=$r['chat_type']==='supergroup'?'Супергруппа':'Группа';
        $s.="{$status} <b>{$title}</b>\n🆔 <code>{$r['chat_id']}</code>\n📁 {$type}\n🕐 ".date('d.m.Y H:i',(int)$r['last_seen_at'])."\n\n";
    }
    return $s;
}

function adminChatsKeyboard(): ?array {
    global $db;
    $rows=$db->query("SELECT chat_id,title,username,active FROM chat_registry WHERE chat_type IN ('group','supergroup') ORDER BY active DESC,last_seen_at DESC LIMIT 30")->fetchAll();
    if(!$rows) return null;
    $items=[];
    foreach($rows as $r){
        $title=$r['title']?:($r['username']?'@'.$r['username']:(string)$r['chat_id']);
        $title=mb_substr((string)$title,0,24);
        $items[]=[((int)$r['active']===1?'🚫 ':'🟢 ').$title,((int)$r['active']===1?'adm:chatoff:':'adm:chaton:').(int)$r['chat_id']];
    }
    $result=[];
    foreach(array_chunk($items,2) as $chunk) $result[]=$chunk;
    $result[]=[['🔄 Обновить','adm:chats']];
...
function profile(int $id): string {
    $u=user($id);
    $vip=isVip($id) ? '💎 VIP до: <b>'.date('d.m.Y H:i',(int)$u['vip_until']).'</b>' : '💎 VIP: <b>нет</b>';
    return "👤 <b>ПРОФИЛЬ</b>\n\n".
...
if (isset($update['callback_query'])) {
    ...
    if ($a==='admin') {
        ...
        send($chat,"👑 <b>АДМИН-ПАНЕЛЬ</b>",buttons([
            ...
            ['💬 Чаты бота','adm:chats'],['👑 Администраторы','adm:admins']
        ]));
        exit;
    }
    ...
    if (str_starts_with($a,'adm:') && admin($id)) {
        ...
        if (str_starts_with($act,'chatoff:')) {
            $chatId=(int)substr($act,8);
            $db->prepare('UPDATE chat_registry SET active=0 WHERE chat_id=?')->execute([$chatId]);
            answer($cb,'Чат исключён из рассылки');
            send($chat,adminChatsText(),adminChatsKeyboard());
            exit;
        }
        if (str_starts_with($act,'chaton:')) {
            $chatId=(int)substr($act,7);
            $db->prepare('UPDATE chat_registry SET active=1,last_seen_at=? WHERE chat_id=?')->execute([time(),$chatId]);
            answer($cb,'Чат включён в рассылку');
            send($chat,adminChatsText(),adminChatsKeyboard());
            exit;
        }
        if ($act==='stats') {
...
if ($text==='/start') {
    $isNewUser=ensureUser($from);
    registerChat($m['chat']);
    trackChatActivity($id,$chat,$m['chat']['type']??'');
    if ($isNewUser) {
        send($chat,"🚨 <b>ДОБРО ПОЖАЛОВАТЬ В «РАБОТА БАНДИТА»!</b> 🚨\n\n💰 Здесь ты будешь зарабатывать бабки, выполнять работы, грабить игроков и развивать свой бизнес.\n\n💼 <b>Работа</b> — стабильный заработок.\n🚨 <b>Ограбления</b> — риск ради большой награды.\n🏪 <b>Мои бизнесы</b> — управление своим магазином.\n🎉 <b>События</b> — актуальные розыгрыши и награды.\n💎 <b>VIP</b> — специальные задания и награды.\n\n🔥 Начни с раздела «Работа» и развивай своего бандита!",mainMenu());
    } else {
...
switch($text) {
    ...
    case '🏪 Мои бизнесы':
        $q=$db->prepare('SELECT 1 FROM shops WHERE user_id=?'); $q->execute([$id]);
        $hasShop=(bool)$q->fetchColumn();
        send($chat, $hasShop ? '🏪 <b>МОИ БИЗНЕСЫ</b>\n\nТвой магазин доступен ниже.' : '🏪 <b>МОИ БИЗНЕСЫ</b>\n\nУ тебя пока нет бизнеса.\nКупи «Свой магазин», чтобы открыть бизнес.', buttons($hasShop ? [['🏪 Свой магазин','shop']] : [['🏪 Купить свой магазин','buy:shop']]));
        break;
    case '🚨 Ограбить':
