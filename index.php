...
function send(int $chat, string $text, ?array $keyboard=null): array {
    // Convert accidental literal escape sequences into real Telegram line breaks.
    $text = str_replace(['\\r\\n','\\n','\\r'], ["\r\n","\n","\r"], $text);
...
function ensureUser(array $from): bool {
    ...
    $store->update('users',function($users)use($from,$id,$now,&$new){
        ...
        $new=true;$users[]=['id'=>$id,'username'=>$from['username']??null,'first_name'=>$from['first_name']??'','balance'=>START_BALANCE,'respect'=>0,'vip_until'=>0,'vip_started_at'=>0,'vip_tier'=>0,'strength'=>START_STRENGTH,'level'=>START_LEVEL,'xp'=>START_XP,'registered_at'=>$now,'last_seen_at'=>$now,'last_daily_at'=>0];return $users;
    },[]);
    return $new;
}
...
function vipTiers(): array {
    return [
        1 => ['name'=>'VIP Уличный','icon'=>'🔪','cost'=>200,'work'=>10,'rob'=>5,'shop'=>0,'xp'=>0,'daily'=>0],
        2 => ['name'=>'VIP Авторитет','icon'=>'🔫','cost'=>500,'work'=>20,'rob'=>10,'shop'=>0,'xp'=>0,'daily'=>1],
        3 => ['name'=>'VIP Криминальный босс','icon'=>'💰','cost'=>1000,'work'=>30,'rob'=>15,'shop'=>20,'xp'=>0,'daily'=>1],
        4 => ['name'=>'VIP Главарь','icon'=>'👑','cost'=>2000,'work'=>45,'rob'=>20,'shop'=>30,'xp'=>10,'daily'=>1],
        5 => ['name'=>'VIP Легенда','icon'=>'💎','cost'=>5500,'work'=>60,'rob'=>25,'shop'=>50,'xp'=>25,'daily'=>1],
    ];
}
function vipTier(int $id): int { $u=getUser($id); if(!$u || (int)($u['vip_until']??0)<=time()) return 0; $tier=(int)($u['vip_tier']??1); return isset(vipTiers()[$tier])?$tier:1; }
function isVip(int $id): bool { return vipTier($id)>0; }
function vipInfo(int $id): array { $tier=vipTier($id); return $tier?vipTiers()[$tier]:[]; }
function vipWorkMultiplier(int $id): float { $v=vipInfo($id); return 1+((int)($v['work']??0)/100); }
function vipRobberyMultiplier(int $id): float { $v=vipInfo($id); return 1+((int)($v['rob']??0)/100); }
function vipShopMultiplier(int $id): float { $v=vipInfo($id); return 1+((int)($v['shop']??0)/100); }
function vipXpMultiplier(int $id): float { $v=vipInfo($id); return 1+((int)($v['xp']??0)/100); }
function addVipXp(int $id,int $amount): void { addXp($id,(int)floor($amount*vipXpMultiplier($id))); }
function vipDescription(): string {
    return "💎 <b>VIP СИСТЕМА</b>

💎 Респекты: <b>11.300</b>
⭐️ 1 Telegram Star = 50 Респектов
💳 Для пополнения: @hixub

".
        "✅ 🔪 <b>VIP Уличный</b> — <b>200</b> Респектов
Работа +10%
Ограбление +5%

".
        "✅ 🔫 <b>VIP Авторитет</b> — <b>500</b> Респектов
Работа +20%
Ограбление +10%
Доп. ежедневный бонус

".
        "✅ 💰 <b>VIP Криминальный босс</b> — <b>1000</b> Респектов
Работа +30%
Ограбление +15%
Доход магазина +20%

".
        "✅ 👑 <b>VIP Главарь</b> — <b>2000</b> Респектов
Работа +45%
Ограбление +20%
Доход магазина +30%
XP +10%
...
".
        "➡️ 💎 <b>VIP Легенда</b> — <b>5500</b> Респектов
Работа +60%
Ограбление +25%
Доход магазина +50%
XP +25%
...
function profile(int $id): string { $u=getUser($id);$tier=vipTier($id);$v=vipInfo($id);$vip=$tier?'💎 '.esc($v['name']).' до: <b>'.date('d.m.Y H:i',(int)$u['vip_until']).'</b>':'💎 VIP: <b>нет</b>';return "👤 <b>ПРОФИЛЬ</b>\n\n💰 Бабки: <b>".money((int)$u['balance'])."</b>\n💎 Респекты: <b>".money((int)$u['respect'])."</b>\n{$vip}\n💪 Сила: <b>{$u['strength']}/100</b>\n⭐ Уровень: <b>{$u['level']}</b>\nXP: <b>{$u['xp']}/".xpNeed((int)$u['level'])."</b>\n\n🆔 ID: <code>{$id}</code>"; }
...
if(isset($update['callback_query'])){
    ...
    if($a==='upgrade'){ $u=getUser($id);$cost=1000*((int)$u['strength']+1);if((int)$u['strength']>=MAX_STRENGTH){answer($cb,'Максимальная сила');exit;}if(!changeBalance($id,-$cost,'Прокачка силы')){answer($cb,'Недостаточно денег');exit;}changeUser($id,function($u){$u['strength']++;return $u;});addVipXp($id,10);answer($cb,'Сила увеличена!');send($chat,'💪 Сила увеличена!\n\n'.profile($id));exit; }
    ...
    if(str_starts_with($a,'adm:')&&admin($id)){ $act=substr($a,4);
        ...
        if($act==='vip'){send($chat,'💎 <b>УПРАВЛЕНИЕ VIP</b>\n\n/premka ID УРОВЕНЬ ДНИ\nНапример: <code>/premka 123456789 5 30</code>\n\nУровни: 1 — Уличный, 2 — Авторитет, 3 — Криминальный босс, 4 — Главарь, 5 — Легенда\n\n/unvip ID\n/respect ID +/-SUM');exit;}
...
if(preg_match('~^/(?:premka|vip)\s+(\d+)\s+(\d+)\s+(\d+)$~u',$text,$mm)&&admin($id)){ $target=(int)$mm[1];$tier=(int)$mm[2];$days=max(1,(int)$mm[3]);$tiers=vipTiers();if(!getUser($target)){send($chat,'❌ Пользователь не найден.');exit;}if(!isset($tiers[$tier])){send($chat,'❌ Уровень VIP должен быть от 1 до 5.');exit;}$until=time()+$days*86400;changeUser($target,function($u)use($until,$tier){$u['vip_until']=$until;$u['vip_started_at']=time();$u['vip_tier']=$tier;return $u;});global $store;$store->update('vip_claims',fn($r)=>array_values(array_filter($r,fn($x)=>(int)$x['user_id']!==$target)),[]);$v=$tiers[$tier];send($chat,'✅ <b>Премиум выдан!</b>\n\n'.$v['icon'].' '.$v['name'].'\n🆔 <code>'.$target.'</code>\n⏰ На: <b>'.$days.' дн.</b>\n📅 До: <b>'.date('d.m.Y H:i',$until).'</b>');send($target,'💎 <b>ТЕБЕ ВЫДАН VIP!</b>\n\n'.$v['icon'].' <b>'.$v['name'].'</b>\n⏰ До: <b>'.date('d.m.Y H:i',$until).'</b>');exit;}
if(preg_match('~^/unvip\s+(\d+)$~',$text,$mm)&&admin($id)){changeUser((int)$mm[1],function($u){$u['vip_until']=0;$u['vip_started_at']=0;$u['vip_tier']=0;return $u;});send($chat,'💎 VIP снят.');exit;}
...
case '🎁 Бонус':$u=getUser($id);if(time()-(int)$u['last_daily_at']<86400){send($chat,'🎁 Бонус уже получен. Возвращайся позже.');break;}$daily=DAILY_BONUS+(vipInfo($id)['daily']??0)*DAILY_BONUS;changeBalance($id,$daily,'Ежедневный бонус');changeUser($id,function($u){$u['last_daily_at']=time();return $u;});addVipXp($id,15);send($chat,'🎁 Ежедневный бонус: 💰 <b>'.money($daily).'</b>');break;
case '💼 Работа':$jobs=getJobs();$last=0;foreach($jobs as$r)if((int)$r['user_id']===$id)$last=max($last,(int)$r['created_at']);if(time()-$last<WORK_COOLDOWN){send($chat,'💼 Работа пока недоступна. Попробуй позже.');break;}$reward=random_int(2000,6000);$reward=(int)floor($reward*vipWorkMultiplier($id));changeBalance($id,$reward,'Работа');addVipXp($id,20);addJob($id,$reward);$v=vipInfo($id);$bonus=((int)($v['work']??0)>0?'\n💎 VIP-бонус: +'.$v['work'].'%':'');send($chat,'💼 Работа выполнена!\n💰 +<b>'.money($reward).'</b>'.$bonus.'\n⭐ +20 XP');break;
